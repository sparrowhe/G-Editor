...
//----------
	public function stopAll():void {
		...
		clearRunFeedback();
		app.stagePane.resetCloneIDs();
		//app.stagePane.deleteClones();
		cloneCount = 0;
		clearKeyDownArray();
		ScratchSoundPlayer.stopAllSounds();
		app.extensionManager.stopButtonPressed();
		//remove stage var printing stuff
		var s2:ScratchObj = app.stagePane;
		var i:int = 0;
		for (i = 0; i < s2.variables.length; i++)
			if (s2.variables[i].txt){
				app.stagePane.addChild(s2.variables[i].txt);
				app.stagePane.removeChild(s2.variables[i].txt);
			}
		//end of removing stage var printing stuff
		app.stagePane.clearFilters();
		for each (var s:ScratchSprite in app.stagePane.sprites()) {
			s.clearFilters();
			s.hideBubble();
			//remove sprite var printing stuff
			for (i = 0; i < s.variables.length; i++)
			if (s.variables[i].txt){
				app.stagePane.addChild(s.variables[i].txt);
				app.stagePane.removeChild(s.variables[i].txt);
			}
			//end of removing sprite var printing stuff
		}
		clearAskPrompts();
