...
public class LibraryPart extends UIPart {
	...
	private const bgColor:int = 0xf9f9f9;//0xe6e8e8;
	...
	public function LibraryPart(app:Scratch) {
		...
		spritesTitle = makeLabel(Translator.map('Sprites'), CSS.titleFormat, stageAreaWidth + 10, 2);
	...
	private function fixLayout():void {
		...
		newSpriteLabel.x = libraryButton.x - newSpriteLabel.width +18;//-6 //+18是右移标签
		newSpriteLabel.y = 2;
	...
	private function addNewBackdropButtons():void {
		...
		var buttonY:int = 145;
		backdropLibraryButton.x = -20000;
		backdropLibraryButton.y = buttonY + 3;
		backdropPaintButton.x = backdropLibraryButton.right() + 19992;//12
		backdropPaintButton.y = buttonY + 1;
		backdropImportButton.x = backdropPaintButton.right() + 7;
		backdropImportButton.y = buttonY + 0;
		backdropCameraButton.x = backdropImportButton.right() + 9;
		backdropCameraButton.y = buttonY + 1;
/*		这里是最左边一条添加背景button的代码
		var buttonY:int = 145;
