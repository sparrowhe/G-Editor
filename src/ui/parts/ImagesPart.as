...
public class ImagesPart extends UIPart {
	...
	private function addNewCostumeButtons():void {
		var left:int = 8;
		var buttonY:int = 32;
		addChild(backdropLibraryButton = makeButton(costumeFromLibrary, 'library', /*left*/left+20000, buttonY + 1));
		addChild(costumeLibraryButton = makeButton(costumeFromLibrary, 'library', /*left + 1*/left+20000, buttonY - 2));
		addChild(paintButton = makeButton(paintCostume, 'paintbrush', left+5/* + 23*/, buttonY - 1));
		addChild(importButton = makeButton(costumeFromComputer, 'import', left+35 /*+ 44*/, buttonY - 2));
		addChild(cameraButton = makeButton(costumeFromCamera, 'camera', left +65/*+ 72*/, buttonY));
	}
	...
	private function fixEditorLayout():void {
		...
		// import button
		libraryButton.x = clearButton.x + clearButton.width + smallSpace+20000;//加20000是移出边界外
		libraryButton.y = clearButton.y;
		editorImportButton.x = libraryButton.x-20000 /*- libraryButton.width */+smallSpace;
		editorImportButton.y = clearButton.y;

		// flip and costume center buttons
		flipHButton.x = editorImportButton.x + editorImportButton.width + bigSpace+libraryButton.width;//libraryButton.width是加上去的
