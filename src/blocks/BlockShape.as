...
public class BlockShape extends Shape {
	...
	/*	var f:BevelFilter = new BevelFilter(1);
		f.blurX = f.blurY = 4;//3
//		f.type=BitmapFilterType.INNER;
		f.highlightColor=0x000000;
		f.highlightAlpha = 0.3;//0.3 这里是更改模块顶上白色光影效果的
		f.shadowAlpha = 0.3;//0.6 这里是更改模块阴影大小的*/
		var f:GlowFilter=new GlowFilter(0x000000, 0.1,2, 2, 10, 1, true, false);
		return [f];
	}
	...
	private function drawHatShape(g:Graphics):void {
		...
		/*	g.moveTo(0, 7);
	//	curve(0, 12, 40, 0, 0.8);
	//	curve(90, 0, 160, 10, 0.8);
		g.lineTo(w - CornerInset, 7);
		g.lineTo(w, 7 + CornerInset);
		drawRightAndBottom(g, topH, true);*/
	}
/*	g.moveTo(0, 12);
		curve(0, 12, 40, 0, 0.15);
	...
	private function drawProcHatShape(g:Graphics):void {
		const trimColor:int = 0xff6680;//0x8E2EC2; // 0xcf4ad9;
