...
public class BlockArg extends Sprite {
	...
	public function BlockArg(type:String, color:int, editable:Boolean = false, menuName:String = '') {
		...
		if (type == 'c') {
			...
			base.setWidthAndTopHeight(30, Block.argTextFormat.size + 6.5); // 15 for normal arg font //6
		}
		...
		if (editable || isNumber || (type == 'm')) { // add a string field
			...
			else base.setWidthAndTopHeight(30, Block.argTextFormat.size + 8); // 14 for normal arg font//5
	...
	private function blockArgFilters():Array {
		...
		f.blurX = f.blurY = 2;
		f.highlightAlpha = 0;//0.3 这里是模块输入框和下拉框等的顶部白色光影效果
		f.shadowAlpha = 0;//0.6 这里是模块输入框和下拉框等的阴影
