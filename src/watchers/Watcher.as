...
public class Watcher extends Sprite implements DragClient {
	...
	private const format:TextFormat = new TextFormat(CSS.font, 10.5, 0, true);
	...
	public function Watcher() {
		frame = new ResizeableFrame(/*0x949191*/0xd4d5d6, /*0xC1C4C7*/0xd4d5d6, 8);
		addChild(frame);
		addLabel();
	...
	public function initForVar(target:ScratchObj, varName:String):void {
		...
		setColor(/*Specs.variableColor*/ /*0x0099DD*//*0x3e464c*/0x4c97ff);
	...
	private function addSliderAndKnob():void {
		...
		f.angle = 225;
		f.shadowAlpha = 0;//0.5 变量框的阴影
		f.highlightAlpha = 0;//0.5变量框的白色光影效果
		slider.filters = [f];
		addChild(slider);

		knob = new Shape();
		var g:Graphics = knob.graphics;
		g.lineStyle(1, 0xa2a3a6);
		g.beginFill(0xa2a3a6/*FFFFFF*/);
		g.drawCircle(5, 5, 5);
	//	f=new int(0xa2a3a6)
	//	f = new BevelFilter(2);
	/*	f.blurX = f.blurY = 0;
		f.highlightColor=0xa2a3a6;
		f.shadowColor=0xa2a3a6;*/
		
	/*	f.shadowColor=0xa2a3a6;
		f.highlightColor=0xa2a3a6;*/
	//	knob.filters = /*[f]*/0xa2a3a6;
		addChild(knob);
	}
