...
public class ListWatcher extends Sprite {
	...
	public function ListWatcher(listName:String = 'List Title', contents:Array = null, target:ScratchObj = null, limitView:Boolean = false) {
		...
		frame = new ResizeableFrame(/*0x949191, 0xC1C4C7*/0xd4d5d6,0xd4d5d6, 14, false, 2);
...
//		}
		...
		if(!contents.length) {
			var tf:TextField = createTextField(Translator.map('空'), cellNumFont);
	...
	private function updateElementCount():void {
		elementCount.text = /*Translator.map('共')  +*/ contents.length+'项';
